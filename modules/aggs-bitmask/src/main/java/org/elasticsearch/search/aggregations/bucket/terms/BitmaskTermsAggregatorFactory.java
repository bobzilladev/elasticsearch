...
public class BitmaskTermsAggregatorFactory extends ValuesSourceAggregatorFactory<ValuesSource, BitmaskTermsAggregatorFactory> {
    private static final DeprecationLogger DEPRECATION_LOGGER = new DeprecationLogger(Loggers.getLogger(BitmaskTermsAggregatorFactory.class));
    ...
    BitmaskTermsAggregatorFactory(String name,
    ...
    private static boolean isAggregationSort(BucketOrder order) {
        if (order instanceof InternalOrder.Aggregation) {
            ...
            return compoundOrder.orderElements().stream().anyMatch(BitmaskTermsAggregatorFactory::isAggregationSort);
        } else {
            return false;
        }
    }
